...
<thead>
  ...
  <tr id={{"tr".$session->id}}>
    <form action="" id="{{$session->id}}">
        ...
        <td  id={{"session".$session->id}} contenteditable="false">
                
            @if (!in_array([$session->id],$sessionsA))          
            <a id ={{"session".$session->id}} onclick="create_session_a_year({{$academic_yearId}},{{$session->id}})">
